...
export const deleteComment = createAsyncThunk(
  "tour/deleteComment",
  async ({ tourId, commentId, toast }, { rejectWithValue }) => {
    try {
      const response = await api.deleteComment(tourId, commentId);
...
export const addComment = createAsyncThunk(
  ...
  async ({ id, comment, toast }, { rejectWithValue }) => {
    try {
      const response = await api.commentTour(id, comment);
  ...
  extraReducers: {
    ...
    [getRelatedTours.rejected]: (state, action) => {
      ...
      [addComment.fulfilled]: (state, action) => {
        state.loading = false;
        state.tour = action.payload;
        state.tour.comments = action.payload.comments; // Assure la mise à jour des commentaires
